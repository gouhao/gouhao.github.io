...
struct page *alloc_pages_current(gfp_t gfp, unsigned order)
{
	...
	if (pol->mode == MPOL_INTERLEAVE)
		...
		page = __alloc_pages_nodemask(gfp, order,
				// 一般情况下都返回当前节点，除非指定了策略里指定了MPOL_BIND
				policy_node(gfp, pol, numa_node_id()),
				// 一般情况下都返回NULL，只有在MPOL_BIND才返回指定的node集合
...
struct mempolicy *get_task_policy(struct task_struct *p)
{
	...
	if (node != NUMA_NO_NODE) {
		/* 
		numa_policy_init是在numa_policy_init里初始化的，
		...
		unsigned int *alloc_flags)
{
	...
	// 在gfp_mask里有一些位是用来存储zone类型的
	ac->highest_zoneidx = gfp_zone(gfp_mask);
	// preferred_nid是首选的zone节点，这个算出zone节点对应的zonelist
	// 这个返回的是ZONELIST_FALLBACK类型的zonelist
	...
	// cpu集合情况。todo: 后面再看这个路径，这个一般是false
	if (cpusets_enabled()) {
...
struct zoneref *__next_zones_zonelist(struct zoneref *z,
					...
					nodemask_t *nodes)
{
	// 能走到这个函数一般都是指定了nodesmask，所以这里用unlikely
...
alloc_flags_nofragment(struct zone *zone, gfp_t gfp_mask)
{
	...
	if (nr_online_nodes > 1 && !populated_zone(--zone))
		return alloc_flags;
	// 走到这儿表示：只有一个node或者normal zone里有可用的页面
	// 则不允许混合页类型
...
static inline int gfp_migratetype(const gfp_t gfp_flags)
{
	...
	// 一般不会走这个分支, 在内存较少的系统上page_group_by_mobility_disabled是1,
	// 表示可移动性被禁用，因为在这种系统上打开这个特性不会有什么好处
...
retry:
	...
	for_next_zone_zonelist_nodemask(zone, z, ac->highest_zoneidx,
		...
		if (!zone_watermark_fast(zone, order, mark,
			...
			// ALLOC_NO_WATERMARKS表示不检查水印，如果有这个标志，再试一下当前结点
			if (alloc_flags & ALLOC_NO_WATERMARKS)
				goto try_this_zone;

			// 当前结点不允许回收？
			if (node_reclaim_mode == 0 ||
				// zone_allows_reclaim是判断首选节点和zone节点的距离 <= node_reclaim_distance,
				// node_reclaim_distance默认是30
				// 距离大于node_reclaim_distance，则不回收。
				// todo: why ?
...
try_this_zone:
	...
	// 不允许回退，则去掉ALLOC_NOFRAGMENT，再试一次
	if (no_fallback) {
		alloc_flags &= ~ALLOC_NOFRAGMENT;
		goto retry;
	}
				...
				unsigned int alloc_flags, gfp_t gfp_mask)
{
	...
	if (!order) {
		...
		// __zone_watermark_unusable_free计算不可用的空闲页，要从所有空闲页里减去这些页
		fast_free -= __zone_watermark_unusable_free(z, 0, alloc_flags);
		// 如果空闲的大于水印和保留的值，则内存可用，可以分配
		if (fast_free > mark + z->lowmem_reserve[highest_zoneidx])
			return true;
	}

	// 如果上面快速路径没有满足要求，则具体检查水印值，或者在高阶上能否分配
	if (__zone_watermark_ok(z, order, mark, highest_zoneidx, alloc_flags,
					free_pages))
		return true;

	 // 0 order, GFP_ATOMIC类型的页，使用min水印，再试一次。
			 ...
			 long free_pages)
{
	...
	// 判断当前zone能否满足大于1页的请求,在这里如果指定的order不符合要求
	// 则会判断比它大的order.
	for (o = order; o < MAX_ORDER; o++) {
		...
		// 小于MIGRATE_PCPTYPES的迁移类型：不可移动，可移动，可回收
		// 这三种类型都是使用较多的。
		for (mt = 0; mt < MIGRATE_PCPTYPES; mt++) {
			// 只要有一个迁移类型有空闲页，就返回 true
			if (!free_area_empty(area, mt))
				return true;
		}

#ifdef CONFIG_CMA
		// 如果是分配CMA，要判断CMA迁移类型是否有空闲页
		if ((alloc_flags & ALLOC_CMA) &&
		    !free_area_empty(area, MIGRATE_CMA)) {
			return true;
		}
#endif
		// 对于harder请求，需要MIGRATE_HIGHATOMIC有空闲
		// todo: what is MIGRATE_HIGHATOMIC?
