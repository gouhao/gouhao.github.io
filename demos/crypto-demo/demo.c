...
static int get_random_bytes(u8 *buf, unsigned int len)
    {
             ...
             unsigned char *digest)
{
    ...
    char *hash_alg_name = "sha1-generic";
    int ret;
...
static int __init crypto_test_init(void)
{
    ...
    err = test_hash("gouhao", 6, digest);
    if (err)
        printk(KERN_ERR "test hash failed!\n");
    else
	    printk("digest=%s\n", digest);
	err =  test_skcipher();
	if (err)
		printk("test skcipher failed!\n");
	else
		printk("skcipher success!\n");
